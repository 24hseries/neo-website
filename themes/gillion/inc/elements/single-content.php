...
class vcg_single_content extends WPBakeryShortCode {

    function __construct() {
        add_action( 'init', array( $this, '_mapping' ), 12 );
        add_shortcode( 'vcg_single_content', array( $this, '_html' ) );
    }
    ...
    public function _mapping() {
        ...
        vc_map(
            array(
                'name' => __('Single Content', 'jevelin'),
                'base' => 'vcg_single_content',
                //'description' => __('Blank space with custom height', 'jevelin'),
                'category' => __('Gillion Single Elements', 'jevelin'),
    ...
    public function _html( $atts ) {
        ...
        ob_start(); ?>
            ...
            <div class="sh-element-post-single">
                <p>
                    <?php
                        if( is_single() && !isset( $_GET['vc_editable'] ) ) :
                            wp_reset_postdata();
                            the_content();
                        else :
                            esc_attr_e( 'Single content', 'jevelin' );
                        endif;
                    ?>
                </p>
            </div>
